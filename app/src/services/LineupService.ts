...
import type { Lineup, Member } from "@/types";
...
class LineupService {
   ...
   */
  update(id: string, data: Partial<Lineup>): Promise<Lineup> {
  ...
  filterRejectedProposals(
    proposals: PositionProposal[],
    rejectedPositionProposals: PositionProposal[][]
  ): PositionProposal[] {
    function isProposalNotRejected(proposal: PositionProposal[]) {
      const alreadyRejected = rejectedPositionProposals.some(
        (rejectedProposal: PositionProposal[]) => {
          return proposal.every((p: PositionProposal) =>
            rejectedProposal.some(
              (rp: PositionProposal) =>
                Math.abs(rp.x - p.x) < 1 && Math.abs(rp.y - p.y) < 1
            )
          );
        }
      );
    ...
    if (Array.isArray(proposals[0]))
      return (proposals as unknown as PositionProposal[][]).filter(
        (proposal: PositionProposal[]) => isProposalNotRejected(proposal)
      ) as unknown as PositionProposal[];
    else return isProposalNotRejected(proposals) ? proposals : [];
  }

  getLinesForNumberOfMembers(n: number): number[] | undefined {
    switch (n) {
      case 8:
        return [3, 2, 3];
      case 9:
  ...
  proposeLineup(
    ...
    function createLinePositions(members: Member[], lineNumber = 0) {
      const spacing = 100 / 7;
    ...
    if (teamMembers.length === 0 || teamMembers.length === 1) {
      ...
      if (
        lines!.reduce((partialSum: number, a: number) => partialSum + a, 0) !=
        teamMembers.length
      ) {
        throw Error(
      ...
      const linePositions = lines!.map(
        (lineLength: number, lineNumber: number) => {
