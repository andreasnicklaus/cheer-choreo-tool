...
import type { Choreo, Lineup, Member, MatTypeGroup } from "@/types";
...
class ChoreoService {
   ...
   */
  getByTeam(teamId: string): Promise<Choreo[]> {
   ...
   */
  getById(choreoId: string): Promise<Choreo> {
   ...
   */
  changeName(choreoId: string, name: string): Promise<Choreo> {
   ...
   * @returns {Promise<Object>} Updated choreography
   */
  changeLength(choreoId: string, counts: number): Promise<Choreo> {
   ...
   * @returns {Promise<Object>} Updated choreography
   */
  changeMatType(choreoId: string, matType: string): Promise<Choreo> {
   ...
   * @returns {Promise}
   */
  removeParticipant(choreoId: string, MemberId: string): Promise<void> {
    debug(`Removing participant ${MemberId} from choreo ${choreoId}`);
    return ax
      .delete(`/choreo/${choreoId}/participants/${MemberId}`)
      .then((res) => res.data);
  }
   ...
   * @returns {Promise}
   */
  remove(choreoId: string): Promise<void> {
   ...
   */
  getPositionsFromChoreoAndCount(
    choreo: Choreo,
    count: number,
    teamMembers: Member[]
  ): PositionData[] {
    ...
    const positionsForCurrentCount = relevantLineups
      .map((l: Lineup) => l.Positions!)
      .flat();

    let unPositionedTeamMembers = teamMembers.filter(
      (m: Member) => !positionsForCurrentCount.some((p) => p.MemberId == m.id)
    );

    const interpolatedPositions: PositionData[] = [];
    unPositionedTeamMembers.forEach((member: Member) => {
      const lineupsForMember = choreo.Lineups!.filter(
        (l: Lineup) =>
          l.Positions && l.Positions.some((p) => p.MemberId == member.id)
      );

      const previousLineupForMember = lineupsForMember
        .filter((l: Lineup) => l.endCount < count)
        .sort((a: Lineup, b: Lineup) => b.endCount - a.endCount)[0];

      const followingLineupForMember = lineupsForMember
        .filter((l: Lineup) => l.startCount > count)
        .sort((a: Lineup, b: Lineup) => a.startCount - b.startCount)[0];
    ...
    const defaultPositions = unPositionedTeamMembers.map(
    ...
    return [
      ...
      .map((p: PositionData) => ({
        ...p,
        Member: teamMembers.find((tm: Member) => tm.id == p.MemberId),
      }))
