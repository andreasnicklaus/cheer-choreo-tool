...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const extraData: {
      ...
      clubList?: entityList;
      teamList?: entityList;
      seasonTeamList?: entityList;
      matTypeList?: entityList;
    } = {
      userList: (await UserService.getAll()).map((user: User) => ({
        value: user.id,
        name: user.username,
      })),
    };
    switch (req.params.entity) {
      ...
      case "choreos":
        ...
        extraData.matTypeList = (
          Choreo.rawAttributes.matType.values as MatType[]
        ).map((matType) => ({
          value: matType,
          name: matType,
        }));
    ...
    if (searchTerm)
      data = search(data, { keys: columns }, searchTerm as string);
    ...
    res.render("../src/views/admin/db.ejs", {
      ...
      editColumns: JSON.stringify(
        Object.entries(
          (
            model as {
              getAttributes: () => Record<string, { _autoGenerated?: boolean }>;
            }
          ).getAttributes(),
        )
          ...
          .map(([key, value]) => ({
            column: key,
            type: sequelizeDataTypeToHtmlInputType(
              (value as { type: { key: string } }).type,
            ),
          })),
      ),
